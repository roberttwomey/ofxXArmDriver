...
//int* GetFreeParameters() { static int freeparams[] = {5}; return freeparams; }
...
int GetNumFreeParameters() { return 1; }
int* GetFreeParameters() { static int freeparams[] = {6}; return freeparams; }
...
bool ComputeIk2(const IkReal* eetrans, const IkReal* eerot, const IkReal* pfree, IkSolutionListBase<IkReal>& solutions, void* pOpenRAVEManip) {
    IKSolver solver;
    return solver.ComputeIk(eetrans,eerot,pfree,solutions);
}

const char* GetKinematicsHash() { return "b38bae72e6f105bd9572e8ac73a65e61"; }

const char* GetIkFastVersion() { return "0x1000004a"; }
...
//     // ComputeFk(const IkReal* j, IkReal* eetrans, IkReal* eerot) {
    ...
    for(int i=0; i< 3;++i){
       T6[i*4+0] = eerot[i*3+0];
       T6[i*4+1] = eerot[i*3+1];
       T6[i*4+2] = eerot[i*3+2];
       T6[i*4+3] = eetrans[i];
    }
    T6[3*4+0] = 0;
    T6[3*4+1] = 0;
